...
import { FileText, Plus, Pencil, Trash2, X, Save, Eye, EyeOff, Calendar } from "lucide-react";

interface Post {
  id: string; title: string; slug: string; excerpt: string; content: string;
  image: string; tags: string; published: boolean; publishedAt: string | null;
}
const EMPTY: Omit<Post, "id"> = {
  title: "", slug: "", excerpt: "", content: "", image: "", tags: "", published: false, publishedAt: null,
};
...
export default function AdminBlogPage() {
  ...
  async function load() { const r = await fetch("/api/admin/blog"); setPosts(await r.json()); }
  useEffect(() => { load(); }, []);

  function openCreate() { setForm(EMPTY); setMsg(""); setOpen(true); }
  function openEdit(p: Post) {
    setForm({ ...p, tags: JSON.parse(p.tags || "[]").join(", ") });
    setMsg(""); setOpen(true);
  }
  function set(key: keyof typeof form, val: string | boolean | null) { setForm((p) => ({ ...p, [key]: val })); }
  ...
  async function save() {
    ...
    setSaving(false);
    if (res.ok) { setOpen(false); load(); } else { const d = await res.json(); setMsg(d.error || "Erreur."); }
  }
  ...
  return (
    <div>
      <div className="flex items-start justify-between mb-8">
        <div>
          <div className="flex items-center gap-2 mb-1">
            <FileText size={14} strokeWidth={1.75} className="text-zinc-500" />
            <p className="text-[11px] font-semibold text-zinc-600 uppercase tracking-[0.14em]">Contenu</p>
          </div>
          <h1 className="text-2xl font-bold text-zinc-100 tracking-tight">Blog</h1>
          <p className="text-zinc-600 text-sm mt-1">{posts.length} article{posts.length !== 1 ? "s" : ""}</p>
        </div>
        <button onClick={openCreate}
          ...
          <Plus size={14} strokeWidth={2.5} /> Nouvel article
        </button>
      </div>
      ...
      <div className="space-y-2">
            ...
            <div key={p.id}
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="text-xs text-zinc-600">/blog/{p.slug}</span>
                  {p.publishedAt && (
        ...
        {posts.length === 0 && (
          <div className="py-20 text-center text-zinc-600 text-sm">Aucun article. Créez-en un !</div>
        )}
      </div>

      <Modal open={open} onClose={() => setOpen(false)}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.07] shrink-0">
          <h2 className="text-sm font-bold text-zinc-100">{form.id ? "Modifier l'article" : "Nouvel article"}</h2>
        ...
        <div className="overflow-y-auto px-6 py-5 space-y-4 flex-1">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input className={inputCls} value={form.slug} onChange={(e) => set("slug", toSlug(e.target.value))} placeholder="mon-article" />
            </div>
            <div>
            ...
            <div className="col-span-2">
              ...
              <textarea className={textareaCls} rows={2} value={form.excerpt} onChange={(e) => set("excerpt", e.target.value)} placeholder="Résumé de l'article..." />
            </div>
            <div className="col-span-2">
              <label className={labelCls}>Contenu Markdown</label>
              <textarea className={textareaCls} rows={12} value={form.content} onChange={(e) => set("content", e.target.value)} placeholder="# Mon article&#10;&#10;Contenu..." />
            </div>
            <div className="col-span-2">
            ...
            <div className="col-span-2 flex items-center gap-3">
              <button type="button" onClick={() => setForm((p) => ({
                ...
                publishedAt: !p.published && !p.publishedAt ? new Date().toISOString() : p.publishedAt,
              }))}
                className={`relative w-10 h-5.5 rounded-full transition-colors ${form.published ? "bg-emerald-500" : "bg-white/[0.1]"}`}
                style={{ height: "22px" }}>
                <span className={`absolute top-0.5 w-4.5 h-4.5 rounded-full bg-white shadow transition-transform ${form.published ? "translate-x-5" : "translate-x-0.5"}`}
                  style={{ width: "18px", height: "18px", transform: form.published ? "translateX(20px)" : "translateX(2px)" }} />
              </button>
